...
import { formatDateRange, statusLine, tripDays } from './packingExport';
...
function weatherBlock(model: ExportModel): string {
  ...
  if (!options.includeWeather || (!trip.weatherConditions && !trip.weatherNotes && trip.weatherLow == null)) return '';
...
export function buildPrintableHtml(model: ExportModel): string {
  ...
  return `
    <div class="page">
      <div class="banner">
        <h1>🧽 Spongie Ultimate Travel Packing List</h1>
      ...
      <div class="footerNote">Generated by Spongie · ${model.generatedAt.toLocaleString()}</div>
    </div>
  `;
}
...
<head>
<meta charset="utf-8" />
<title>${esc(model.trip.name)} — Spongie Packing List</title>
<style>
  ${DOC_STYLE}
  @media print {
