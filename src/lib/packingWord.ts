...
import { statusLine, departureCountdown, conditionEmoji } from './packingExport';
...
export async function buildTripDocx(model: ExportModel): Promise<Blob> {
  ...
  children.push(new Paragraph({
    ...
    spacing: { after: 100 },
    children: [new TextRun({ text: '🧽 ULTIMATE TRAVEL PACKING LIST', bold: true, size: 40, color: PURPLE })],
  }));
  children.push(new Paragraph({
    alignment: AlignmentType.CENTER,
    spacing: { after: 100 },
  ...
  children.push(new Paragraph({
    alignment: AlignmentType.CENTER,
    spacing: { after: 300 },
    children: [new TextRun({ text: departureCountdown(trip), bold: true, size: 20, color: GREY })],
  }));
  ...
  if (options.includeWeather && (trip.weatherConditions || trip.weatherNotes || trip.weatherLow != null || hasDaily)) {
  ...
  children.push(new Paragraph({
    alignment: AlignmentType.CENTER,
    spacing: { before: 200 },
    children: [new TextRun({ text: `Generated on ${model.generatedAt.toLocaleString()}`, italics: true, size: 18, color: GREY })],
  }));
...
export async function buildMasterDocx(model: MasterExportModel): Promise<Blob> {
  ...
  children.push(new Paragraph({
    alignment: AlignmentType.CENTER,
    spacing: { after: 100 },
    children: [new TextRun({ text: '🧽 MASTER PACKING LIBRARY', bold: true, size: 36, color: PURPLE })],
  }));
  children.push(new Paragraph({
    alignment: AlignmentType.CENTER,
    spacing: { after: 300 },
